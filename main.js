...
class SidebarView extends ItemView {
  ...
  createWordCard(wordObj, container) {
    ...
    contentContainer.setText("加载释义中...");
  ...
  async focusWord(wordObj, preferredSource = null) {
    ...
    this.activeTab = targetCard.mastered ? "mastered" : "learning";
    this.filterWords();
    this.render();
...
class ConfirmModal extends Modal {
  ...
  onOpen() {
    ...
    const confirmBtn = buttonDiv.createEl("button", { text: "确认", cls: "mod-cta" });
...
class WordModal extends Modal {
  ...
  onOpen() {
    ...
    new Setting(contentEl).setName("音标").addText(text => {
...
class WordbookSettingTab extends PluginSettingTab {
  ...
  buildFilesTab(container) {
    ...
    for (let idx = 0; idx < this.plugin.settings.wordbookFiles.length; idx++) {
      ...
      const updateLock = () => {
        ...
        lockIcon.title = isReadonly ? '点击切换为可写' : '点击切换为只读';
      };
      updateLock();
    ...
    new Setting(container)
      ...
      .addDropdown(async (drop) => {
        drop.addOption("per-source", "按词源独立").addOption("global", "全局统一")
          ...
          .onChange(async (val) => {
            ...
            await this.plugin.highlighter.refresh();
            this.plugin.app.workspace.trigger("simple-wordbook:data-updated");
            const modeLabel = val === "global" ? "全局统一" : "按词源独立";
  ...
  buildGeneralTab(container) {
    ...
    new Setting(container).setName(t("settings_scope_highlight")).setDesc("开启后，高亮仅作用于匹配路径的文件；关闭后，高亮作用于所有文件").addToggle(toggle => toggle.setValue(this.plugin.settings.enableHighlightScopeFilter).onChange(async (val) => {
      this.plugin.settings.enableHighlightScopeFilter = val;
      await this.plugin.saveSettings();
      await this.plugin.highlighter.refresh();
      this.plugin.app.workspace.trigger("simple-wordbook:data-updated");
    }));
    new Setting(container).setName(t("settings_scope_sidebar")).setDesc("开启后，侧边栏仅显示匹配路径的文档中的单词；关闭后，显示所有文档的单词").addToggle(toggle => toggle.setValue(this.plugin.settings.enableSidebarScopeFilter).onChange(async (val) => {
    ...
    new Setting(container).setName(t("settings_scope_paths")).setDesc(t("settings_scope_paths_desc")).addTextArea(textarea => {
      ...
      textarea.inputEl.placeholder = "文件夹/笔记.md\n文件夹";
    });
  ...
  buildAITab(container) {
    ...
    new Setting(container)
      ...
      .addText(text => {
        ...
        text.setPlaceholder(settings.apiProvider === "custom" ? "请输入 API 地址" : "自动填充");
        text.onChange(async (val) => {
